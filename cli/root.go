// Package cli builds the ct command tree on top of the clinicaltrials library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: clinicaltrials.DefaultConfig()}

	root := &cobra.Command{
		Use:   "ct",
		Short: "Search ClinicalTrials.gov clinical studies",
		Long: `ct reads the ClinicalTrials.gov registry through the public REST API v2.
No API key is required. It returns records as table, JSON, JSONL, CSV, TSV, or URLs.

ct is an independent tool and is not affiliated with the US National Library of Medicine or NIH.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = clinicaltrials.NewClient(a.cfg)
	return nil
}
