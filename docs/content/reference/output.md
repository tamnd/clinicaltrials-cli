...
describes what users get. Pick a format with `-o`, or let ct choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ct <command> -o table   # aligned columns for reading
ct <command> -o jsonl   # one JSON object per line, for piping
ct <command> -o json    # a single JSON array
ct <command> -o csv     # spreadsheet friendly
ct <command> -o tsv     # tab-separated
ct <command> -o url     # just the URL column
ct <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ct <command> --fields id,title,url
```
...
```bash
ct <command>            # a table, because this is a terminal
ct <command> | wc -l    # JSONL, because this is a pipe
```
